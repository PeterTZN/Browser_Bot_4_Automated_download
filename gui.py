...
class App:
    def __init__(self, root):
        ...
        self.entry_current_address.grid(row=2, column=1)
        ...
        self.entry_permanent_address.grid(row=3, column=1)

        # Buttons
        self.button_frame = tk.Frame()
        self.button_frame.pack(padx=10, pady=10)

        tk.Button(self.button_frame, text="Submit", command=self.submit_data).grid(row=0, column=0, padx=5, pady=5)
        tk.Button(self.button_frame, text="Close Browser", command=self.close_browser).grid(row=0, column=1, padx=5, pady=5)
